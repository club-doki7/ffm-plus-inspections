...
public final class FfmPlusMagicCompletionContributor extends CompletionContributor implements DumbAware {
  ...
  public void fillCompletionVariants(final @NotNull CompletionParameters parameters, final @NotNull CompletionResultSet result) {
    ...
    FfmPlusMagicConstantUtils.AllowedValues allowedValues = getAllowedValues(pos);
    if (allowedValues == null) return;

    addCompletionVariants(parameters, result, pos, allowedValues);
  }

  public static @Nullable FfmPlusMagicConstantUtils.AllowedValues getAllowedValues(@NotNull PsiElement pos) {
    FfmPlusMagicConstantUtils.AllowedValues allowedValues = null;
    for (Pair<PsiModifierListOwner, PsiType> pair : getMembersWithAllowedValues(pos)) {
      FfmPlusMagicConstantUtils.AllowedValues values = FfmPlusMagicConstantUtils.getAllowedValues(pair.first, pair.second, pos);
      ...
      if (exp != null && (exp.getOperationTokenType() == JavaTokenType.EQEQ || exp.getOperationTokenType() == JavaTokenType.NE)) {
        for (PsiExpression operand : exp.getOperands()) {
          ...
          if (resolved != null) {
            ...
            FfmPlusMagicConstantInspection.processValuesFlownTo(operand, pos.getContainingFile(), pos.getManager(), true, expression -> {
  ...
  private static void addCompletionVariants(final @NotNull CompletionParameters parameters,
                                            ...
                                            PsiElement pos,
                                            FfmPlusMagicConstantUtils.AllowedValues allowedValues) {
