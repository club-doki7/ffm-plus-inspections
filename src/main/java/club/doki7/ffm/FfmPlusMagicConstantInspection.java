...
import club.doki7.ffm.FfmPlusMagicConstantUtils.AllowedValues;
...
public final class FfmPlusMagicConstantInspection extends AbstractBaseJavaLocalInspectionTool {
  ...
  private static void checkExpression(@NotNull PsiExpression expression,
    ...
    AllowedValues allowed = FfmPlusMagicConstantUtils.getAllowedValues(owner, type, expression);
  ...
  private static void checkCall(@NotNull PsiCall methodCall, @NotNull ProblemsHolder holder) {
    ...
    for (int i = 0; i < parameters.length; i++) {
      ...
      AllowedValues values = FfmPlusMagicConstantUtils.getAllowedValues(parameter, type, methodCall);
  ...
  private static @Nullable LocalQuickFix suggestMagicConstant(@NotNull PsiExpression argument,
    ...
    if (!allowedValues.isFlagSet()) {
      ...
      for (PsiAnnotationMemberValue value : allowedValues.getValues()) {
        if (value instanceof PsiExpression expression) {
          Long constantValue = FfmPlusMagicConstantUtils.evaluateLongConstant(expression);
  ...
  private static boolean isGoodExpression(@NotNull PsiExpression argument,
    ...
    if (allowedValues.isFlagSet()) {
      ...
      if (FfmPlusMagicConstantUtils.same(argument, zero, manager)
      ...
      if (FfmPlusMagicConstantUtils.same(argument, minusOne, manager)) return true;
    ...
    if (allowedForRef == null && owner != null) {
      allowedForRef = FfmPlusMagicConstantUtils.getAllowedValues(owner, PsiUtil.getTypeByPsiElement(owner), argument);
    }
  ...
  private static boolean isOneOf(@NotNull PsiExpression expression, @NotNull AllowedValues allowedValues, @NotNull PsiManager manager) {
    return ContainerUtil.exists(allowedValues.getValues(), e -> FfmPlusMagicConstantUtils.same(e, expression, manager));
  }
